...
class MQSub(MQBase):
    """This is an example consuming that will handle unexpected interactions
    ...
    def __init__(self,
        ...
        """Create a new instance of the consuming class, passing in the AMQP
    ...
    def declare_pub_and_sub_models(self):
        for sub in self.sub_models.values():
            ...
            self.logger.debug(f"Declaring exchange {sub.exchange}")
        ...
        for pub in self.pub_models.values():
            self.logger.debug(f"Setting up sub model {pub}")
            self.logger.debug(f"Declaring exchange {pub.exchange}")
