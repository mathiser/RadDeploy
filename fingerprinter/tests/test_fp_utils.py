...
def test_generate_df_from_tar(scp_tar):
    ...
    assert "dcm_path" in df.columns
    assert df["dcm_path"].tolist() == ['ct.dcm', 'mr.dcm']
...
def test_generate_flow_specific_tar(scp_tar):
    ...
    with open("t.tar", "wb") as f:
        f.write(sliced_tar.read())
    sliced_df = generate_df_from_tar(sliced_tar)
    assert sliced_df["path"].tolist() == ["MR_1.3.46.670589.11.71891.5.0.8000.2022062210324747248_1.3.46.670589.11.71891.5.0.8000.2022062210385801250.dcm"]
